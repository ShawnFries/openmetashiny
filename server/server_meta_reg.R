...
extract.cov.data <- function(reg.data, dont.make.array=F) {
...
res <- rma.uni(yi=reg.data@y, sei=vals$datar$sdi, slab=vals$datar$names, level=conflevel, digits=input$digits, method=params$rm.method, mods=cov.vals)

output$msummary_reg <- renderPrint({print(res)})